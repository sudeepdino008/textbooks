...
(define (car-stream exp) (cadr exp))
(define (cdr-stream exp) (make-lambda '() (caddr exp)))

(define (car-stream? exp) (tagged-list? exp 'car-stream))
(define (cdr-stream? exp) (tagged-list? exp 'cdr-stream))

(define (cons-stream->cons exp)
  (let ((var (list 'cons (car-stream exp) (cdr-stream exp))))
    (display var)
    var)
  )
...
(define (analyze exp)
  ...
  (cond ((self-evaluating? exp) (analyze-self-evaluating exp))
        ...
        ((cdr-stream? exp) (analyze-cdr-stream exp))
        ((car-stream? exp) (analyze-car-stream exp))
...
;;(define (car/cdr-stream-body exp) (cadr exp))
(define (car-stream-arguments exp) (cadr exp))
(define (analyze-car-stream exp)
  (let ((body (analyze (car-stream-arguments exp))))
    ;;(display body)
    (lambda (env) (car (body env)))
    ))

(define (analyze-cdr-stream exp)
  (let ((body (analyze (car-stream-arguments exp))))
    (lambda (env)
      ;;(display body)
...
(define (analyze-if exp)
  (let ((predicate (analyze (if-predicate exp)))
    ...
    (lambda (env)
      (if (true? (predicate env)) (consequent env) (alternate env)))
    )
  )
...
;;(trace analyze-car-stream)
;;(trace analyze-cdr-stream)
;;(trace analyze)
;;(trace analyze-application)
;;(trace apply-primitive-procedure)
;; (trace car-stream-arguments)
