...
(define (make-let initializations body)
  (list 'let initializations body))
...
(define (stream-cdr exp)
  '(let ((already-run? false) (result false))
     (lambda ()
       (if (not already-run?)
           (begin (set! result (cddr exp))
                  (set! already-run? true)
                  result)
           result))))

  ;; (make-lambda '() (list '(let ((already-run? false) (result false))
  ;;                           (lambda ()
  ;;                             (if (not already-run?)
  ;;                                 (begin (set! result (cddr exp))
  ;;                                        (set! already-run? true)
...
(trace analyze-assignment)
(trace analyze-definition)
(driver-loop)


((lambda (already-run? result) (lambda () (if (not already-run?) (begin (set! result (cddr exp)) (set! already-run? true) result) result))) false false)
